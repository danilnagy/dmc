...
Finally, we pass the scaled data set to the `.predict()` method of the model, which returns a predicted value for the target. In this case, it will return its best guess for the price that an apartment in this grid cell will cost. We will store this value into the same 'grid' double list that we used to store the values of the heat map. This way, the rest of our code should work in the same way. 
