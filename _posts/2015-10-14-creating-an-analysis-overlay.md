...
This is the current request being sent to the server every time the `updateData()` function runs. At this point it is only sending the latitude and longitude ranges of the area to use for querying the data points. In place of this line, let's create a few new variables to store the dimensions of our browser window as well as our desired grid cell size, and modify the request to send this information to the server:
