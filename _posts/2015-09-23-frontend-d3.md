...
Now that we have our client-side code complete, we will go back to the `app.py` file to update our server with the new route, and specify the data that gets sent back to the client. In the `app.py` file, under the definition of the main route, write the following code:
